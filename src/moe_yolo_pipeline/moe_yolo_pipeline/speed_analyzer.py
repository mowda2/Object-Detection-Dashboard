...
# TMC / Counting integration
from .counting import CountingLine, LineCrossingDetector, CrossingEvent
from .turn_movements import TurnMovementClassifier, CompletedMovement
from .interval_binning import IntervalAggregator
from .report_generator import generate_tmc_pdf, generate_utdf_csv, generate_speed_summary_csv
...
def run_speed_job(job_dir: str, settings: dict = None, progress_cb=None, message_cb=None) -> dict:
    ...
    # ---------------------------------------------------------------------------
    # Load counting lines for TMC integration (optional – additive feature)
    # ---------------------------------------------------------------------------
    counting_lines_cfg = cal_data.get("counting_lines", [])
    counting_lines = []
    for cl in counting_lines_cfg:
        try:
            counting_lines.append(CountingLine(
                name=cl["name"],
                x1=int(cl["x1"]), y1=int(cl["y1"]),
                x2=int(cl["x2"]), y2=int(cl["y2"]),
                direction=cl["direction"],
            ))
        except (KeyError, ValueError) as e:
            message_cb(f"Warning: Skipping invalid counting line: {e}")
    
    tmc_enabled = len(counting_lines) > 0
    if tmc_enabled:
        message_cb(f"TMC enabled: {len(counting_lines)} counting line(s) loaded")
    
    # Study start time (maps video second 0 to a wall-clock time)
    study_start_time = effective_settings.get("study_start_time", "00:00")
    ...
    # ---------------------------------------------------------------------------
    # TMC / Counting pipeline setup (additive – runs alongside speed pipeline)
    # ---------------------------------------------------------------------------
    line_detector = None
    turn_classifier = None
    interval_agg = None
    all_crossing_events: List[CrossingEvent] = []
    all_completed_movements: List[CompletedMovement] = []
    # Per-track current speed lookup for crossing events
    current_speeds: Dict[int, float] = {}
    
    if tmc_enabled:
        line_detector = LineCrossingDetector(counting_lines)
        turn_classifier = TurnMovementClassifier(timeout_s=30.0)
        interval_agg = IntervalAggregator(study_start_time=study_start_time)
        message_cb(f"TMC pipeline initialised (study start: {study_start_time})")
        ...
        while True:
            ...
            for i in range(len(detections)):
                ...
                if track_id >= 0:
                    ...
                    # Store speed for TMC crossing events
                    if tmc_enabled:
                        current_speeds[track_id] = speed_kph
            ...
            # ---------------------------------------------------------------------------
            # TMC: Update counting line detector with tracked detections
            # ---------------------------------------------------------------------------
            if tmc_enabled and line_detector is not None:
                crossing_events = line_detector.update(
                ...
                for cev in crossing_events:
                    all_crossing_events.append(cev)
                    if cev.object_class == "Pedestrian":
                        interval_agg.add_pedestrian(cev)
                    elif cev.object_class == "Bicycle":
                        interval_agg.add_bicycle(cev)
                    else:
                        result_mv = turn_classifier.process_crossing(cev)
                        if result_mv is not None:
                            all_completed_movements.append(result_mv)
                            interval_agg.add_movement(result_mv)
    ...
    # ---------------------------------------------------------------------------
    # TMC post-processing: flush pending, compute results, generate reports
    # ---------------------------------------------------------------------------
    tmc_summary = {"enabled": tmc_enabled}
    
    if tmc_enabled and turn_classifier is not None and interval_agg is not None:
        # Flush any tracks that entered but never exited
        video_duration_s = frame_idx / fps if fps > 0 else 0.0
        flushed = turn_classifier.flush_all(video_duration_s)
        for mv in flushed:
            all_completed_movements.append(mv)
            interval_agg.add_movement(mv)
        
        message_cb(f"TMC: {len(all_crossing_events)} crossings, "
                    f"{len(all_completed_movements)} movements "
                    f"({len(flushed)} flushed)")
        
        # Compute interval results
        intersection_name = effective_settings.get("intersection_name", "Intersection")
        study_date = effective_settings.get("study_date", datetime.now().strftime("%Y-%m-%d"))
        
        tmc_result = interval_agg.compute_results(intersection_name, study_date)
        
        # Generate report files alongside existing outputs
        report_files = {}
        try:
            # Build speed data list for the PDF speed summary page
            speed_data_for_pdf = []
            for cev in all_crossing_events:
                if cev.object_class not in ("Pedestrian", "Bicycle") and cev.speed_kmh > 0:
                    speed_data_for_pdf.append({
                        "Track_ID": cev.track_id,
                        "Class": cev.object_class,
                        "Speed_KMH": cev.speed_kmh,
                        "Timestamp_S": cev.timestamp_s,
                        "Approach": cev.direction,
                    })
            
            pdf_path = os.path.join(job_dir, "tmc_report.pdf")
            generate_tmc_pdf(tmc_result, pdf_path, speed_data=speed_data_for_pdf or None)
            report_files["pdf"] = "tmc_report.pdf"
            message_cb("TMC PDF report generated")
        except Exception as e:
            message_cb(f"Warning: PDF generation failed: {e}")
        
        try:
            utdf_path = os.path.join(job_dir, "volume_utdf.csv")
            generate_utdf_csv(tmc_result, utdf_path)
            report_files["utdf_csv"] = "volume_utdf.csv"
            message_cb("UTDF CSV generated")
        except Exception as e:
            message_cb(f"Warning: UTDF CSV generation failed: {e}")
        
        try:
            speed_csv_path = os.path.join(job_dir, "speed_summary.csv")
            # Build full speed data from per_track stats for the speed summary CSV
            speed_records = []
            for cev in all_crossing_events:
                if cev.speed_kmh > 0:
                    speed_records.append({
                        "Track_ID": cev.track_id,
                        "Class": cev.object_class,
                        "Speed_KMH": round(cev.speed_kmh, 1),
                        "Timestamp_S": round(cev.timestamp_s, 2),
                        "Approach": cev.direction,
                    })
            if speed_records:
                generate_speed_summary_csv(speed_records, speed_csv_path)
                report_files["speed_csv"] = "speed_summary.csv"
                message_cb("Speed summary CSV generated")
        except Exception as e:
            message_cb(f"Warning: Speed summary CSV generation failed: {e}")
        
        # Serialise TMC bins for dashboard display
        tmc_bins_json = []
        for b in tmc_result.bins:
            tmc_bins_json.append({
                "interval_start": b.interval_start,
                "approach": b.approach,
                "movement": b.movement,
                "vehicle_counts": b.vehicle_counts,
                "total": b.total,
                "heavy_vehicle_count": b.heavy_vehicle_count,
                "pedestrian_count": b.pedestrian_count,
                "bicycle_count": b.bicycle_count,
            })
        
        tmc_summary.update({
            "total_crossings": len(all_crossing_events),
            "total_movements": len(all_completed_movements),
            "flushed_incomplete": len(flushed),
            "report_files": report_files,
            "tmc_data": tmc_bins_json,
            "peak_hours": {
                "am_peak_hour": tmc_result.am_peak_hour,
                "am_peak_volume": tmc_result.am_peak_volume,
                "am_peak_phf": tmc_result.am_peak_phf,
                "pm_peak_hour": tmc_result.pm_peak_hour,
                "pm_peak_volume": tmc_result.pm_peak_volume,
                "pm_peak_phf": tmc_result.pm_peak_phf,
            },
            "total_volume": tmc_result.total_volume,
            "heavy_vehicle_pct": tmc_result.heavy_vehicle_pct,
            "intersection_name": tmc_result.intersection_name,
            "study_date": tmc_result.study_date,
            "study_start": tmc_result.study_start,
            "study_end": tmc_result.study_end,
        })
