...
def _parse_settings_from_form(form) -> Dict[str, Any]:
    ...
    # ---------------------------------------------------------------------------
    # TMC / Traffic study settings
    # ---------------------------------------------------------------------------
    if form.get("study_start_time"):
        settings["study_start_time"] = form["study_start_time"].strip() or "00:00"
    else:
        settings["study_start_time"] = "00:00"
    
    if form.get("intersection_name"):
        settings["intersection_name"] = form["intersection_name"].strip()
    else:
        settings["intersection_name"] = "Intersection"
...
def speed_calibrate_submit(job_id: str):
    ...
    width_m = float(request.form.get("width_m", 3.5))
    height_m = float(request.form.get("height_m", 10))
    calibration_data = {
    ...
    # ---------------------------------------------------------------------------
    # Parse counting lines from form (TMC integration)
    # ---------------------------------------------------------------------------
    counting_lines_json = request.form.get("counting_lines_json", "")
    if counting_lines_json:
        try:
            cl_list = json.loads(counting_lines_json)
            # Validate each entry
            valid_lines = []
            for cl in cl_list:
                if all(k in cl for k in ("name", "x1", "y1", "x2", "y2", "direction")):
                    if cl["direction"] in ("NB", "SB", "EB", "WB"):
                        valid_lines.append({
                            "name": str(cl["name"]),
                            "x1": int(cl["x1"]),
                            "y1": int(cl["y1"]),
                            "x2": int(cl["x2"]),
                            "y2": int(cl["y2"]),
                            "direction": str(cl["direction"]),
                        })
            calibration_data["counting_lines"] = valid_lines
        except (json.JSONDecodeError, TypeError, ValueError):
            pass  # silently skip invalid counting lines
    else:
        # Preserve existing counting lines if they were already saved
        existing_cal = _load_calibration(job_id)
        if existing_cal and "counting_lines" in existing_cal:
            calibration_data["counting_lines"] = existing_cal["counting_lines"]
    
    # ---------------------------------------------------------------------------
    # Parse study_start_time from form
    # ---------------------------------------------------------------------------
    study_start_time = request.form.get("study_start_time", "").strip()
    if study_start_time:
        calibration_data["study_start_time"] = study_start_time
    else:
        # Preserve existing value or use default
        existing_cal = _load_calibration(job_id)
        if existing_cal and "study_start_time" in existing_cal:
            calibration_data["study_start_time"] = existing_cal["study_start_time"]
        else:
            calibration_data["study_start_time"] = "07:00"
...
def speed_artifact(job_id: str, filename: str):
    ...
    # For CSV and PDF, set as attachment for download
    as_attachment = filename.endswith(".csv") or filename.endswith(".pdf")
...
def speed_results(job_id: str):
    ...
    # Extract TMC data from summary for template
    tmc = summary.get("tmc", {}) if summary else {}
